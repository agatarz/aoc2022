...
class Monkey:
    ...
    def inspect_item(self, print_flag=True):

        item = self.first_item()
        worry_item = self.worry_level_operation(item)
        bored_item = worry_item // 3
        ...
        if print_flag:
            ...
            print('  Worry level: %d.' % worry_item)
            print('  Monkey gets bored with item. Worry level is divided by 3 to %d' % bored_item)
            print('  is diversible ', diversibility)
            print('  throw to: %d' % next_monkey)
    ...
    for round in range(20):
        ...
        for monkey in monkeys:
            for i in range(len(monkey.items)):
                throw_to_monkey, item = monkey.inspect_item(False)
...
assert solution(input_puzzle(test_file)) == 10605
